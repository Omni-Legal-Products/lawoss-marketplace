...
- `eurlex-celex`: `node ${workspaceFolder}/dist/server.js`
...
3. HTTP uses `API_TOKEN` (bearer authentication), not the OAuth settings used by other LAWOSS servers. Set `TRANSPORT=http`, an explicit `PORT`, and `ALLOWED_ORIGINS` to the host allowlist. EUR-Lex web-service credentials are separate upstream credentials; they do not authenticate MCP clients.
