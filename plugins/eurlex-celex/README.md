...
Public CELEX document retrieval works without credentials. EUR-Lex account credentials may be required for account-backed search services. Responses use bounded text windows.
...
Refresh the marketplace with `codex plugin marketplace upgrade lawoss`, then reinstall `eurlex-celex@lawoss`. The [central updater](../../docs/updating.md) can update selected installed LAWOSS plugins together. Start a new task after updating. Local plugin updates do not deploy hosted servers.
