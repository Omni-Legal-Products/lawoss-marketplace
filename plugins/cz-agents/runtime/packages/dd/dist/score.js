...
export function evaluateFlags(input) {
    ...
    if (input.adisStatus?.reliability === 'ANO') {
        flags.push({
            ...
            severity: 'high',
            weight: 30,
            description: `Subjekt je v MFČR registru veden jako nespolehlivý plátce DPH${input.adisStatus.unreliable_since ? ` (od ${input.adisStatus.unreliable_since})` : ''}. Platba na nezveřejněný účet zakládá ručení odběratele za daň dle § 109 ZDPH.`,
            source: 'adis',
    ...
    if (input.adisStatus?.subject_type === 'NESPOLEHLIVA_OSOBA') {
        flags.push({
            ...
            severity: 'high',
            weight: 30,
            description: 'Subjekt je veden jako nespolehlivá osoba dle § 106a ZDPH. Identifikovaný před vstupem do registru plátců DPH jako rizikový.',
            source: 'adis',
            evidence: input.adisStatus,
        });
    }
