...
function requireTier(currentTier, required, toolName) {
    const order = ['free', 'compliance', 'agency', 'enterprise'];
    if (order.indexOf(currentTier) >= order.indexOf(required))
        return null;
    return {
        content: [
            {
                ...
                text: JSON.stringify({
                    ...
                    message: `Tool '${toolName}' requires '${required}' tier or higher. Current: '${currentTier}'. Upgrade at https://mcp.example.com`,
...
export function buildDdServer(clients, tier = 'free') {
    const server = new McpServer({
        name: 'cz-agents/dd',
        version: '0.1.0',
    }, {
        capabilities: { tools: {} },
        instructions: 'Czech company due-diligence aggregator. Combines ARES (legal data, statutory body, VAT, bank accounts), ' +
            ...
            'Free tier (basic report) rate-limited; Compliance and Agency tiers (more tools, higher quotas) at https://mcp.example.com',
    });
    server.tool('get_dd_report', 'Generate a complete due-diligence report for a Czech IČO. Returns company facts (name, address, legal form, VAT status, bank accounts), statutory body with per-member sanctions check, and a transparent risk score with all triggered red flags.', {
    ...
    }, { title: 'Get Czech Company Due-Diligence Report', readOnlyHint: true, openWorldHint: true }, async ({ ico, depth }) => {
    ...
    server.tool('get_risk_score', 'Lightweight version of get_dd_report — returns just the numeric score (0-100), risk level, and top triggered red flags. Faster when you only need a yes/no/maybe screen.', {
        ico: z.string().describe('Czech IČO — 7 or 8 digits.'),
    }, { title: 'Get Risk Score', readOnlyHint: true, openWorldHint: true }, async ({ ico }) => {
    ...
    server.tool('get_statutory_chain', 'Surname-based heuristic walk through statutory bodies of related Czech companies. Best for shell-company unwinding in small s.r.o. with RARE surnames. NOT a true UBO source — for actual beneficial ownership use the ESM (evidence skutečných majitelů, separate registry, future @czagents/esm). For boards of large public companies with common Czech surnames (Novák, Zima, Kolář…) results are noisy by design; the tool auto-skips persons whose surname matches >50 companies with a SURNAME_TOO_COMMON note.', {
        ico: z.string().describe('Czech IČO — 7 or 8 digits.'),
        max_depth: z.number().int().min(1).max(5).default(3).describe('Max recursion depth (default 3, hard cap 5).'),
    }, { title: 'Get Statutory Chain (UBO Walk)', readOnlyHint: true, openWorldHint: true }, async ({ ico, max_depth }) => {
    ...
    server.tool('detect_phoenix', 'Detect phoenix company pattern — 3 surface indicators (surname match with prior insolvent director, founding proximity < 12 months to insolvency, NACE sector presence) computable from ARES + ISIR data alone. Returns PhoenixReport with riskScore 0-100. Pro Compliance tier or higher. For 4 additional deep indicators (founder identity, asset transfer, multi-cycle, address continuity) see detect_phoenix_rich in @czagents/ddplus.', {
        ico: z.string().describe('Czech IČO — 7 or 8 digits.'),
    }, { title: 'Detect Phoenix Company Pattern', readOnlyHint: true, openWorldHint: true }, async ({ ico }) => {
    ...
    server.tool('detect_address_crowding', 'Detects "shell-firm hotel" patterns — counts how many companies share the same registered address. ' +
        'Threshold-based risk: 1-9 normal (multi-tenant office), 10-49 mild (legitimate coworking), ' +
        '50-199 medium (virtual office provider), 200+ high (shell-firm hotel). Compliance tier or higher.', { ico: z.string().describe('Czech IČO 7-8 digits') }, { title: 'Detect Address Crowding', readOnlyHint: true, openWorldHint: true }, async ({ ico }) => {
    ...
    server.tool('get_eu_dd_report', 'EU Due-Diligence report for an international company. Input: 20-char LEI code, or company name + optional country. Returns GLEIF entity data (status, address, registration number) plus sanctions screening against EU/OFAC lists. Coverage notes per country included. Note: GLEIF covers mid/large firms with LEI — SMEs may not be found. Pro Compliance tier or higher.', {
    ...
    }, { title: 'EU Due-Diligence Report (GLEIF + Sanctions)', readOnlyHint: true, openWorldHint: true }, async ({ identifier, country }) => {
        ...
        if (isLei) {
            ...
            if (match)
                record = await getByLei(match.lei);
        }
        if (!record) {
            return wrap(JSON.stringify({
                error: 'not_found',
        ...
        if (record.status === 'dissolved')
            riskIndicators.push('ENTITY_DISSOLVED');
        if (sanctionsMatches.length > 0)
            riskIndicators.push(`SANCTIONS_MATCH(${sanctionsMatches.length})`);
        const coverageNotes = [
            'GLEIF: status, address, registration number. No insolvency, VAT, or UBO data.',
        ];
        if (record.country === 'de')
            coverageNotes.push('DE: Full Handelsregister (statutory bodies, filings) not integrated.');
        if (record.country === 'nl')
            coverageNotes.push('NL: Full KvK data (filings, directors) requires paid API.');
        if (record.country === 'pl')
            coverageNotes.push('PL: KRS full record available via get_company(id, "pl").');
        return wrap(JSON.stringify({
            identifier,
            company: record,
            sanctions: {
    ...
    server.tool('get_eu_parent', 'Find the EU/international parent company for a Czech IČO. Looks up the company name in ARES, then searches GLEIF (Global LEI Foundation) for a matching LEI-registered entity. Returns LEI, name, country, and confidence level (HIGH/MEDIUM/LOW). Note: GLEIF covers mid/large international firms; SMEs without an LEI will not be found. Pro Compliance tier or higher.', { ico: z.string().describe('Czech IČO — 7 or 8 digits.') }, { title: 'Get EU Parent Company (ARES→GLEIF)', readOnlyHint: true, openWorldHint: true }, async ({ ico }) => {
