...
export async function buildReport(ico, clients, opts = {}) {
    ...
    if (!basicOnly && clients.isir) {
        await Promise.all(members.map(async (m, i) => {
            ...
            if (!surname || surname.length < 4 || !firstName)
                return;
            const otherIcos = await findOtherCompaniesByFullName(clients.ares, firstName, surname, ico);
            for (const co of otherIcos.slice(0, 5)) {
                ...
                const isReallyStatutory = await verifyPersonIsStatutory(clients.ares, co.ico, firstName, surname);
                if (!isReallyStatutory)
                    continue;
                const sm = screenedMembers[i];
                if (sm) {
                    if (!sm.prior_bankrupt_companies)
                        sm.prior_bankrupt_companies = [];
    ...
    if (!basicOnly && clients.isir?.searchPersonInsolvency) {
        await Promise.all(screenedMembers.map(async (m, i) => {
            if (!m.is_person)
                return;
...
function extractStatutoryMembers(vr) {
    ...
    for (const organ of vr.statutarniOrgany) {
        ...
        for (const m of organ.clenoveOrganu ?? []) {
            ...
            if (!Number.isNaN(ts) && ts > mostRecent)
                mostRecent = ts;
        }
    }
    if (mostRecent > 0)
        out.mostRecentStatutoryChange = new Date(mostRecent).toISOString().slice(0, 10);
    return out;
}
...
async function screenStatutory(members, sanctions) {
    if (!sanctions) {
...
function screenCompany(ico, name, sanctions) {
    if (!sanctions)
        return null;
...
function rebuildSanctionsMatch(s) {
    return {
...
async function findOtherCompaniesBySurname(ares, surname, excludeIco) {
    try {
 ...
 */
async function findOtherCompaniesByFullName(ares, firstName, surname, excludeIco) {
    try {
 ...
 */
async function verifyPersonIsStatutory(ares, ico, firstName, surname) {
    try {
...
async function checkVirtualAddress(ares, subject) {
    const s = subject?.sidlo;
    if (!s?.nazevObce || !s.nazevUlice)
        return false;
    try {
