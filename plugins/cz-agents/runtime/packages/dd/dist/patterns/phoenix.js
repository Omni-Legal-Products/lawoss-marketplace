...
export function detectPhoenix(r) {
    const indicators = [
    ...
    const unavailable = indicators
        ...
        .map((i) => i.code);
    const riskScore = Math.min(100, indicators
 ...
 */
function detectSurnameMatch(members) {
    ...
    const hits = persons.filter((m) => (m.prior_bankrupt_companies?.length ?? 0) >= 1);
    if (hits.length === 0) {
        return {
            code: 'SURNAME_MATCH',
            fired: false,
    ...
    const priorIcos = hits.flatMap((m) => m.prior_bankrupt_companies?.map((p) => p.ico) ?? []);
    return {
        code: 'SURNAME_MATCH',
        fired: true,
        label: INDICATOR_LABELS.SURNAME_MATCH,
        members: hits.map((m) => m.name),
        detail: `${hits.length} statutář${hits.length === 1 ? '' : 'i'} byl${hits.length === 1 ? '' : 'i'} ` +
 ...
 */
function detectFoundingProximity(r) {
        ...
        if (!Number.isNaN(insolStart.getTime())) {
            ...
            if (Math.abs(gapMonths) < 12) {
                ...
                return {
                    ...
                    fired: true,
                    label: INDICATOR_LABELS.FOUNDING_PROXIMITY,
                    detail: `Firma vznikla ${direction} (${r.insolvency.started_on}). ` +
    ...
    const hasPriors = r.statutory_body.some((m) => (m.prior_bankrupt_companies?.length ?? 0) > 0);
    if (!hasPriors) {
        return {
            code: 'FOUNDING_PROXIMITY',
            fired: false,
    ...
    return {
        ...
        fired: false,
        label: INDICATOR_LABELS.FOUNDING_PROXIMITY,
        detail: 'Záznamy o předchozích krachách existují, ale data o ISIR datumech nejsou ' +
...
function detectNaceMatch(r) {
    ...
    return {
        ...
        fired: false,
        label: INDICATOR_LABELS.NACE_MATCH,
        detail: `Aktuální primární NACE sektor: ${primaryNace}. Záznamy o předchozích krachách existují, ` +
