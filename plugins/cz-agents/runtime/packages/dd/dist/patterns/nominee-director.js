...
export function detectNomineeDirector(r) {
    const indicators = [
...
function detectAgeOutlier(members) {
    ...
    if (!hasDobData) {
        return {
            ...
            fired: false,
            label: INDICATOR_LABELS.AGE_OUTLIER,
            detail: 'Datum narození statutářů není dostupné v ARES VR (není veřejně exponováno).',
            available: false,
        };
    }
    if (hits.length === 0) {
...
function detectMultiBoard(members) {
    const persons = members.filter((m) => m.is_person);
    ...
    const heavyMembers = persons.filter((m) => (m.prior_bankrupt_companies?.length ?? 0) >= 3);
    if (heavyMembers.length === 0) {
        return {
            code: 'MULTI_BOARD',
            fired: false,
            label: INDICATOR_LABELS.MULTI_BOARD,
            detail: 'Žádný ze statutářů nemá 3+ známých konkurzních firem. Plný počet aktivních firem (≥20) vyžaduje ARES OpenData dump (roadmap).',
            available: true,
        };
    }
    return {
        ...
        detail: `${heavyMembers.length} statutář${heavyMembers.length === 1 ? '' : heavyMembers.length < 5 ? 'i' : 'ů'} má 3+ známých ` +
            'konkurzních firem v historii. Aktivní počet firem nelze ověřit bez ARES OpenData — toto je dolní mez.',
        available: true,
    };
}
function detectRecentAppointment(r) {
...
//# sourceMappingURL=nominee-director.js.map