...
async function main() {
    ...
    const clients = { ares, sanctions, isir };
    const server = buildDdServer(clients);
    const transport = new StdioServerTransport();
    await server.connect(transport);
    console.error(`[cz-agents/dd] MCP server ready on stdio (sanctions=${sanctions ? 'enabled' : 'disabled'}, isir=${process.env.ISIR_SOAP_ENABLED ? 'soap' : 'stub'})`);
}
