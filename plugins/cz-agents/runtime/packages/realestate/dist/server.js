 ...
 * See https://mcp.example.com for subscription details.
 *
...
export function buildRealEstateServer(_tier = 'free') {
    const server = new McpServer({
        name: 'cz-agents/realestate',
        version: '0.3.0',
    }, {
        capabilities: { tools: {} },
        instructions: 'Czech distress real estate intelligence — free aggregate tools. ' +
            ...
            'realestate-pro endpoint (https://mcp.example.com) — see https://mcp.example.com',
    });
    // Free tier — k≥3 aggregate, no PII
    server.tool('get_district_aggregate', 'Aggregate distress real estate statistics for a Czech okres (district). Returns counts by category (insolvency / auction) and average market data. Low-volume districts are marked with low_activity. Free tier — no PII exposed.', {
    ...
    }, { title: 'Get District Distress Aggregate', readOnlyHint: true }, async ({ okres, window_days }) => {
