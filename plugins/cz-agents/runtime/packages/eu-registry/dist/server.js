...
export function buildEuRegistryServer(options = {}) {
    const server = new McpServer({
        name: 'cz-agents/eu-registry',
        version: '0.1.0',
    }, {
        capabilities: { tools: {} },
        instructions: 'Non-Czech business registry lookup. Use for companies outside the Czech Republic. ' +
    ...
    server.tool('search_company', 'Search non-Czech business registries by company name. Supported: GB (Companies House), SK (ORSR/RPO), PL (KRS), NL (GLEIF/LEI), DE (GLEIF/LEI), FR (SIRENE).', {
    ...
    }, { title: 'Search Non-Czech Company', readOnlyHint: true, openWorldHint: true }, async ({ name, country, limit }) => {
        ...
        const selected = Object.entries(adapters).filter(([adapterCountry]) => {
            if (normalizedCountry && adapterCountry !== normalizedCountry)
                return false;
            return true;
        });
        const results = await Promise.all(selected.map(async ([, adapter]) => adapter.searchByName(name, cappedLimit)));
    ...
    server.tool('get_company', 'Get a non-Czech company by national ID and country code. Supported: gb (CRN), sk (IČO), pl (KRS number), nl (LEI), de (LEI), fr (SIREN).', {
    ...
    }, { title: 'Get Non-Czech Company', readOnlyHint: true, openWorldHint: true }, async ({ id, country }) => {
        const normalizedCountry = country.toLowerCase();
...
async function getCompany(adapters, id, country) {
    const adapter = adapters[country];
    if (!adapter)
        return null;
    return adapter.getById(id);
}
function buildGleifCache() {
