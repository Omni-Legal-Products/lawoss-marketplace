...
export function buildAdisServer(client = new AdisClient()) {
    const server = new McpServer({
        name: 'cz-agents/adis',
        version: '0.1.0',
    }, {
        capabilities: { tools: {} },
        instructions: 'Czech VAT-payer reliability check (nespolehlivý plátce DPH). Use whenever the user ' +
    ...
    server.tool('check_dph_payer', 'Check VAT-payer reliability for a single Czech subject. Returns reliability status ' +
    ...
    }, { title: 'Check Czech VAT Payer Reliability', readOnlyHint: true, openWorldHint: true }, async ({ ico, dic }) => {
        try {
    ...
    server.tool('check_bulk_dph_payer', `Bulk reliability check for up to ${MAX_DIC_PER_REQUEST} Czech subjects in one ADIS request. ` +
    ...
    }, { title: 'Bulk Check Czech VAT Payer Reliability', readOnlyHint: true, openWorldHint: true }, async ({ icos, dics }) => {
        try {
    ...
    server.tool('list_unreliable_payers', 'Return the full list of currently unreliable Czech VAT payers from ADIS. WARNING: response ' +
        ...
        'check_dph_payer instead.', {}, { title: 'List All Unreliable VAT Payers', readOnlyHint: true, openWorldHint: true }, async () => {
        try {
...
function error(e) {
    ...
    return { content: [{ type: 'text', text: `ADIS query failed: ${msg}` }], isError: true };
}
