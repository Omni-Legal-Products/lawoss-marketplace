 ...
 */
export function buildSanctionsServer(deps) {
    const server = new McpServer({
        name: 'cz-agents/sanctions',
        version: '0.1.0',
    }, {
        capabilities: { tools: {} },
        instructions: 'Sanctions / KYC screening across EU consolidated list and OFAC SDN. ' +
            ...
            '• https://mcp.example.com — Czech insolvency register\n' +
            'Free tier rate-limited; higher limits and commercial AML use at https://mcp.example.com',
    });
    const { db, search } = deps;
    server.tool('search_person', 'Fuzzy-search a sanctioned person by name across all loaded lists. Optional date of birth and nationality narrow results. Returns matches with confidence scores (0-100). 100 = exact ID match, 80+ = strong fuzzy match, lower = review needed.', {
    ...
    }, { title: 'Search Sanctioned Person', readOnlyHint: true, openWorldHint: true }, async ({ name, dob, nationality, threshold, limit }) => {
    ...
    server.tool('search_entity', 'Fuzzy-search a sanctioned entity (company, organization) by name. Optional country narrows results.', {
        ...
        limit: z.number().int().min(1).max(100).default(20).describe('Max results (default 20).'),
    }, { title: 'Search Sanctioned Entity', readOnlyHint: true, openWorldHint: true }, async ({ name, country, threshold, limit }) => {
    ...
    server.tool('check_ico', 'Check whether a Czech IČO (or any company by IČO) appears on sanctions lists. Direct exact-ID lookup; pass `name` to also fuzzy-match if no direct hit.', {
    ...
    }, { title: 'Check IČO Against Sanctions', readOnlyHint: true, openWorldHint: true }, async ({ ico, name }) => {
    ...
    server.tool('get_listing', 'Retrieve the full record for a single sanctions listing by its ID (format: `${source}:${source_list_id}`, e.g. "ofac:12345" or "eu:EU.123.789").', {
        id: z.string().describe('Internal listing ID, e.g. "ofac:12345".'),
    }, { title: 'Get Sanctions Listing Detail', readOnlyHint: true, openWorldHint: true }, async ({ id }) => {
        logToolCall('sanctions', 'get_listing', { id });
        const entity = db.getById(id);
        if (!entity)
            return wrap(`Listing ${id} nenalezen.`);
        return wrap(JSON.stringify(entity, null, 2));
    });
    server.tool('list_recent_updates', 'List sanctions added/removed/modified since a given date. Use for daily monitoring against a watchlist.', {
    ...
    }, { title: 'List Recent Sanctions Updates', readOnlyHint: true, openWorldHint: true }, async ({ since, source }) => {
...
function formatMatches(query, matches) {
    ...
    for (const m of matches) {
        ...
        lines.push(`▸ [${m.confidence}%] ${m.entity.primary_name} (${m.entity.type})`, `   id: ${m.entity.id}`, `   programs: ${m.entity.programs.join(', ') || '-'}`, `   matched_on: ${m.matched_on}${aliasNote}`, '');
    }
    return lines.join('\n');
}
