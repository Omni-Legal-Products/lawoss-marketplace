...
export class SanctionsDb {
    ...
    upsertSource(source, entities) {
        ...
        const existing = this.db
            .prepare('SELECT id, data FROM entities WHERE source = ? AND removed_at IS NULL')
            .all(source);
        const existingMap = new Map(existing.map((row) => [row.id, JSON.parse(row.data)]));
    ...
    `);
        ...
        const insertAlias = this.db.prepare('INSERT OR IGNORE INTO aliases (entity_id, alias, raw_alias) VALUES (?, ?, ?)');
        const deleteIds = this.db.prepare('DELETE FROM ids WHERE entity_id = ?');
        const insertId = this.db.prepare('INSERT OR IGNORE INTO ids (entity_id, type, value, country) VALUES (?, ?, ?, ?)');
        const softDelete = this.db.prepare('UPDATE entities SET removed_at = ? WHERE id = ? AND removed_at IS NULL');
        const insertChange = this.db.prepare('INSERT INTO change_log (ts, entity_id, op, before_data, after_data) VALUES (?, ?, ?, ?, ?)');
        const tx = this.db.transaction(() => {
            for (const e of entities) {
                ...
                const aliasSet = new Set([e.primary_name, ...(e.aliases ?? [])]);
                for (const raw of aliasSet) {
                    const norm = normalizeName(raw);
                    if (norm)
                        insertAlias.run(e.id, norm, raw);
                }
                deleteIds.run(e.id);
    ...
    recordRefreshFailure(source, error) {
        this.db.prepare(`
      INSERT INTO refresh_log (source, refreshed_at, source_count, ok, error)
      VALUES (?, ?, 0, 0, ?)
      ON CONFLICT(source) DO UPDATE SET
    ...
    getById(id) {
        const row = this.db
            .prepare('SELECT data FROM entities WHERE id = ? AND removed_at IS NULL')
            .get(id);
        return row ? JSON.parse(row.data) : null;
    }
    /** Look up by external ID (passport, IČO, tax_id…). Exact match. */
    findByExternalId(type, value) {
        const rows = this.db
            .prepare(`SELECT e.data FROM ids i
     ...
     */
    candidatesByTokens(tokens, typeFilter) {
        if (tokens.length === 0)
            return [];
    ...
    `).all(sinceMs);
        ...
        const lastOpById = new Map();
        for (const row of filtered)
            lastOpById.set(row.entity_id, row);
        for (const row of lastOpById.values()) {
            if (row.op === 'add' && row.after_data) {
                added.push(JSON.parse(row.after_data));
            }
            else if (row.op === 'remove' && row.before_data) {
                removed.push(JSON.parse(row.before_data));
            }
    ...
    stats() {
        const totalActive = this.db.prepare('SELECT COUNT(*) AS c FROM entities WHERE removed_at IS NULL').get().c;
        const totalRemoved = this.db.prepare('SELECT COUNT(*) AS c FROM entities WHERE removed_at IS NOT NULL').get().c;
        const bySource = this.db.prepare('SELECT source, COUNT(*) AS c FROM entities WHERE removed_at IS NULL GROUP BY source').all();
        const refresh = this.db.prepare('SELECT source, refreshed_at, source_count, ok, error FROM refresh_log').all();
        return {
