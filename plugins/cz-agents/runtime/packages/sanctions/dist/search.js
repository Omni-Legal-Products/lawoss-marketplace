    ...
    searchByName(rawName, opts = {}) {
        ...
        for (const ent of candidates) {
            ...
            if (score.confidence < threshold)
                continue;
            if (opts.dob && !dobMatches(ent, opts.dob))
                continue;
            if (opts.nationality && !nationalityMatches(ent, opts.nationality))
                continue;
...
function dobMatches(entity, queryDob) {
    ...
    for (const d of dobs) {
        if (d === queryDob)
            return true;
        if (d.startsWith(queryYear) || queryDob.startsWith(d))
            return true;
    }
    return false;
}
function nationalityMatches(entity, queryNat) {
