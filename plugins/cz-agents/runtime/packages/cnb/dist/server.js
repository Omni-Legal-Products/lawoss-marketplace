...
export function buildCnbServer() {
    const server = new McpServer({
        name: 'cz-agents/cnb',
        version: '0.1.0',
    }, {
        capabilities: { tools: {} },
        instructions: 'Czech National Bank (ČNB) daily FX rates. Use these tools to get official CZK exchange ' +
    ...
    server.tool('get_rates', 'Get official CZK exchange rates published by ČNB. Returns all currencies from the daily sheet (~31 majors). Optional `date` parameter (YYYY-MM-DD) for historical rates; otherwise returns latest.', {
        date: z
            .string()
            .regex(/^\d{4}-\d{2}-\d{2}$/)
            .optional()
            .describe('Optional ISO date (YYYY-MM-DD) for historical rates. Omit for latest.'),
    }, { title: 'Get ČNB Daily FX Rates', readOnlyHint: true, openWorldHint: true }, async ({ date }) => {
    ...
    server.tool('convert', 'Convert an amount between two currencies using official ČNB rates. E.g., convert 100 EUR to CZK, or 50 USD to GBP (goes via CZK cross-rate). Optional `date` for historical conversion.', {
    ...
    }, { title: 'Convert Currency via ČNB Rates', readOnlyHint: true, openWorldHint: true }, async ({ amount, from, to, date }) => {
    ...
    server.tool('get_rate', 'Quick single-currency lookup. Returns just the CZK rate for one currency (or all rates if no code given).', {
        code: z
            .string()
            .length(3)
    ...
    }, { title: 'Get Single Currency ČNB Rate', readOnlyHint: true, openWorldHint: true }, async ({ code, date }) => {
        const sheet = await cnb.getDailyRates(date);
