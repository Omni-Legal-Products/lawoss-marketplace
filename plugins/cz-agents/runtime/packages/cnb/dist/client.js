 ...
 */
const CNB_DAILY = 'https://www.cnb.cz/cs/financni-trhy/devizovy-trh/kurzy-devizoveho-trhu/kurzy-devizoveho-trhu/denni_kurz.txt';
export class CnbClient {
    http;
    // ČNB publishes rates once per business day at ~14:30 CET — cache 10 min is safe
    sheetCache = new TtlCache({
        ttlMs: 10 * 60 * 1000,
        maxSize: 500,
    });
    constructor() {
    ...
    async convert(amount, from, to, date) {
        ...
        const getCzkRate = (code) => {
            if (code === 'CZK')
                return 1;
            const r = sheet.rates.find((x) => x.code === code);
            if (!r)
                throw new Error(`Currency ${code} not found in ČNB sheet (${sheet.date})`);
 ...
 *   "16.04.2026 #73\nzemě|měna|množství|kód|kurz\nAustrálie|dolar|1|AUD|14,132\n..."
 */
export function parseCnbDailyText(text) {
    ...
    if (!m)
        throw new Error(`Unexpected ČNB header: "${lines[0]}"`);
...
function formatCzDate(iso) {
    ...
    if (!m)
        throw new Error(`Date must be YYYY-MM-DD, got "${iso}"`);
    return `${m[3]}.${m[2]}.${m[1]}`;
}
