...
const ipStorage = new AsyncLocalStorage();
...
let _currentIp;
export function setRequestIp(ip) {
    _currentIp = ip;
}
export function clearRequestIp() {
    _currentIp = undefined;
}
export function runWithIp(ip, fn) {
    return ipStorage.run({ ip }, fn);
}
export function getCurrentIp() {
    return ipStorage.getStore()?.ip ?? _currentIp;
}
export function trackIco(ico) {
    if (!isValidIco(ico))
        return;
    ...
    if (icos.size >= MAX_ICOS_PER_IP && !icos.has(ico)) {
        ...
        if (!oldest.done)
            icos.delete(oldest.value);
    }
    icos.add(ico);
    byIp.set(ip, icos);
    icoCounter.set(ico, (icoCounter.get(ico) ?? 0) + 1);
}
export function logToolCall(service, tool, args = {}) {
...
export function getMetrics() {
    ...
    for (const [key, value] of totals) {
        ...
        lines.push(`unique_ico_per_ip_per_day{ip_prefix="${escapeLabel(prefix)}",date="${escapeLabel(date)}"} ${value}`);
    }
    ...
    for (const [key, count] of searchCounter) {
        const [tool, query, city, street] = key.split('\t');
...
function isSafeLogValue(key, value) {
    if (key === 'ico' || key === 'dic')
        return true;
    if (key === 'icos' || key === 'dics')
        return Array.isArray(value);
    if (['depth', 'max_depth', 'since_id', 'limit', 'threshold', 'only_active'].includes(key))
        return true;
    if (['query', 'city', 'street', 'nace', 'psc'].includes(key))
...
//# sourceMappingURL=icoTracker.js.map