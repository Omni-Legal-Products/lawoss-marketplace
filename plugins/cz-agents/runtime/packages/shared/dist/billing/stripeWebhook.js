...
function handleCheckoutCompleted(event, store, config) {
...
function handleSubscriptionChange(event, store) {
...
function handleInvoicePaid(event, store) {
...
function extractPriceFromSession(session) {
 ...
 */
export function verifySignature(rawBody, header, secret, now) {
    ...
    const v1 = parts['v1'];
    if (!t || !v1)
        throw new WebhookError('Malformed Stripe-Signature header', 400);
    const ts = Number(t);
