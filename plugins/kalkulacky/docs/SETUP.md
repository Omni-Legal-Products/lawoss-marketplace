# mcp-kalkulacky-sk · operator setup
...
- `kalkulacky`: `node ${workspaceFolder}/dist/src/server.js`
...
Verified build entrypoint: `node dist/src/server-http.js`.
...
```bash
node --env-file=.env dist/src/server-http.js
```
