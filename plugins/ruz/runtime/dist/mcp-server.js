...
export function createRuzMcpServer(client = new RuzClient()) {
    const server = new McpServer({ name: "ruz-mcp", version: "0.1.0" });
    // Helper: resolve a single entity id from ico/dic, preferring the first.
    async function resolveEntityId(ico, dic) {
        if (ico) {
            const ids = await client.findEntityIdsByIco(ico);
            return ids[0] ?? null;
        }
        if (dic) {
            const ids = await client.findEntityIdsByDic(dic);
            return ids[0] ?? null;
        }
        return null;
    }
    server.registerTool("ruz_find_entity", {
    ...
    server.registerTool("ruz_financial_profile", {
        title: "Finančný profil",
        description: "Viacročný finančný profil subjektu podľa IČO: tržby, výsledok hospodárenia, vlastné imanie, aktíva, záväzky, zadlženosť.",
        inputSchema: {
            ico: z.string(),
        ...
        if (entityId === null)
            return json({ found: false, ico });
        const entity = await client.getEntity(entityId);
        if (entity.stav)
            return json({ found: true, ico, stav: entity.stav, note: "Neverejná jednotka." });
        ...
        for (const stmt of filtered) {
            ...
            const rwts = await Promise.all(reportIds.map(async (rid) => {
    ...
    server.registerTool("ruz_list_statements", {
        ...
        const statements = await Promise.all((entity.idUctovnychZavierok ?? []).map((sid) => client.getStatement(sid)));
        return json({
            found: true,
    ...
    server.registerTool("ruz_get_statement", {
    ...
    server.registerTool("ruz_get_report", {
        title: "Plný účtovný výkaz",
        description: "Plný čitateľný výkaz (súvaha / výkaz ziskov a strát): riadky s názvami a hodnotami za bežné a predchádzajúce obdobie. Pri prázdnom obsahu vráti príznak a odkaz na prílohu.",
        inputSchema: { reportId: looseNumber(z.number().int()) },
    }, async ({ reportId }) => {
        const report = await client.getReport(reportId);
        if (!report.idSablony)
            return json({ id: reportId, error: "no template id" });
    ...
    server.registerTool("ruz_list_annual_reports", {
        ...
        if (id == null)
            return json({ found: false });
        const entity = await client.getEntity(id);
        const reports = await Promise.all((entity.idVyrocnychSprav ?? []).map((aid) => client.getAnnualReport(aid)));
        return json({ found: true, annualReports: reports });
    });
    server.registerTool("ruz_list_attachments", {
        title: "Prílohy",
        ...
        if (statementId != null) {
            ...
            const attachments = reports.flatMap((r) => (r.prilohy ?? []).map((p) => ({ ...p, reportId: r.id })));
    ...
    server.registerTool("ruz_download_attachment", {
