...
export function buildRowIndex(table) {
    const rows = (table.riadky ?? []).map((r, position) => ({
        position,
        cisloRiadku: r.cisloRiadku,
        label: r.text?.sk ?? "",
    }));
    const byCisloRiadku = new Map();
    for (const r of rows)
        byCisloRiadku.set(r.cisloRiadku, r);
    return {
        rows,
        byCisloRiadku,
...
export function cellValue(reportTable, templateTable, rowPosition, col) {
...
export function emptyIndicators() {
...
function classifyTable(name) {
    ...
    if (n.includes("aktív") || n.includes("pasív") || n.includes("súvah"))
        return "suvaha";
    if (n.includes("výnos") ||
        n.includes("náklad") ||
        n.includes("ziskov") ||
 ...
 */
export function extractIndicators(report, template) {
    ...
    for (const def of INDICATORS) {
        if (!reportKinds.has(def.table))
            continue; // indicator belongs to a different report
        let sum = null;
        let firstRow = null;
        if (hasData) {
            for (const p of paired) {
                if (p.kind !== def.table || !p.report)
                    continue;
 ...
 */
export function attachmentHintFor(templateName) {
    ...
    if (name.includes("vybraných údajov") || name.includes("vu pod") || name.includes("vú pod")) {
        return ("Výkaz vybraných údajov (VÚ POD) nemá v RÚZ zverejnený štruktúrovaný obsah " +
...
export function assembleYear(stmt, reports) {
    ...
    const leverageValue = liabilities?.available && assets?.available && assets.value
