...
2. Create an untracked `.env` from the supported variables in `docker-compose.dokploy.yml` and restrict its permissions (`chmod 600 .env`).
3. Use `docker-compose.dokploy.yml` as the environment reference (this edition has no root `.env.example`). Select `MCP_AUTH_MODE=oauth`, set your own `OAUTH_ISSUER_URL` and `OAUTH_AUTHORIZATION_PASSWORD`, and persist `OAUTH_TOKEN_STORE_PATH`. For bearer mode the server uses `MCP_BEARER_TOKEN`, not `MCP_AUTH_TOKEN`.
