...
function parseIPv6(value) {
    ...
    if ((halves.length === 1 && left.length !== 8) || (halves.length === 2 && left.length + right.length >= 8))
        return undefined;
    const words = halves.length === 2 ? [...left, ...Array(8 - left.length - right.length).fill(0), ...right] : left;
    let valueNumber = 0n;
    for (const word of words)
        valueNumber = (valueNumber << 16n) | BigInt(word);
    const mapped = (valueNumber >> 32n) === 0xffffn && (valueNumber >> 48n) === 0n;
    if (mapped)
        return parseIPv4(`${Number((valueNumber >> 24n) & 255n)}.${Number((valueNumber >> 16n) & 255n)}.${Number((valueNumber >> 8n) & 255n)}.${Number(valueNumber & 255n)}`);
...
export function resolveClientAddress(remoteAddress, forwardedFor, trustedProxies) {
    const direct = remoteAddress ? parseIp(remoteAddress) : undefined;
    if (!direct)
        return "unknown";
    if (!trustedProxies.some((cidr) => matches(cidr, direct)))
    ...
    if (forwarded.some((value) => !value))
        return direct.normalized;
    const chain = [...forwarded, direct];
    for (let index = chain.length - 1; index >= 0; index -= 1) {
        const address = chain[index];
        if (!trustedProxies.some((cidr) => matches(cidr, address)))
